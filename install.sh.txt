...
elif [[ -f /etc/kali-release ]] || [[ "$EUID" == "0" ]]; then
...
if [[ "$ENV" == "termux" ]]; then
    ...
    pkg install -y git curl wget python nodejs golang nmap htop
else
    apt update -qq && apt upgrade -y -qq
    apt install -y -qq git curl wget python3 python3-pip golang-go nmap htop tmux
fi

# Project directory
mkdir -p ~/AVS-Pro-v4.0/{config/{wordlists,templates},scans,logs}
cd ~/AVS-Pro-v4.0
...
# Essential tools
if [[ "$ENV" != "termux" ]]; then
    apt install -y -qq masscan nikto gobuster whatweb ffuf sqlmap
    ...
    pip3 install httpx linkfinder
    npm i -g nuclei@latest
fi
